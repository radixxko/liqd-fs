...
function bind_watchers( path, listener, directories, watchers )
{
	for( let directory of directories )
	{
		watchers.set( directory, fs.watch( directory, { persistent: false }, ( event, file ) =>
		{
			fs.stat( directory + file, ( err, stats ) =>
			{
				let watcher = directory + file + '/';

				if( err )
				{
					if( watchers.has( watcher ))
					{
						for( let subwatcher of watchers.keys() )
						{
							if( subwatcher.startsWith(watcher) )
							{
								watchers.get( subwatcher ).close();
								watchers.delete( subwatcher );
							}
						}
					}
				}
				else if( stats.isDirectory() && !watchers.has( watcher ))
				{
					listener( 'rename', ( directory + file ).substr(path.length).replace(/\/$/, ''));
					find( watcher, null, [], ( files ) => files.forEach( file => listener( 'rename', file.substr(path.length).replace(/\/$/, ''))));

					find( watcher, /\/$/, [], ( directories ) =>
					{
						directories.unshift( watcher );

						bind_watchers( path, listener, directories, watchers );
					});
				}
			});

			listener( event, ( directory + file ).substr(path.length) );
		}));
	}
}

function recursive_watch( path, listener )
{
	if( NATIVE_RECURSIVE_WATCH === undefined ){ NATIVE_RECURSIVE_WATCH = require('os').type() !== 'Linux'; }

	if( NATIVE_RECURSIVE_WATCH )
	{
		return fs.watch( path, { persistent: false, recursive: true }, listener );
	}
	else
	{
...
function watch( path, pattern, existing, on_change )
{
	...
	return recursive_watch( path, ( event, file ) =>
	{
...
const FS = module.exports = class FS
{
			...
			Promise.all( searches ).then( () =>
			{
				...
				if( on_change )
				{
					for( let i = 0; i < directories.length; ++i )
					{
						...
						// TODO save watcher and deinit after deletion of directory
	...
	// TODO node 10.12 fs Added a recursive option to fs.mkdir and fs.mkdirSync
